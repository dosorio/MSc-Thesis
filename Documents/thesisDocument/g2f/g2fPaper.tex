...
\caption{Description and comparison of the methods used in the available \texttt{`gapFill'} implemented algorithms under \texttt{`Python'}, \texttt{`GAMS'} and \texttt{`MATLAB'} environment.}\\
\hline
...
\hline
\endhead
\texttt{`SMILEY'} \cite{Reed2006}&\texttt{COBRApy}&\texttt{Python} & {\small Developed by Reed \textit{et al.} (2006), the \texttt{`SMILEY'} algorithm identify through an \emph{optimization algorithm} the minimum number of reactions required to allow the model a selected \textit{metabolite} production from a universal database of stoichiometric reactions. The process is carried out one metabolite per time (user defined). It is an open source implementation under an open source environment.}\\
\hline
\texttt{`gapFind' and `gapFill'} \cite{SatishKumar2007}&\texttt{-}&\texttt{GAMS} &{\small Developed by Kumar \textit{et al.} (2007), the \texttt{`gapFind'} and \texttt{`gapFill'} algorithms identify the metabolites (\texttt{`gapFind'}) in the metabolic network reconstruction which cannot be produced under any uptake conditions in both single and multi-compartment, and subsequently, identify the reactions (\texttt{`gapFill'}) from a customized multi-organism database that restores the connectivity of these metabolites to the original network using a \textit{optimization based} procedures. In the process, the procedures make several intra \emph{model modifications} such as: (1) modify the directionality of the reactions in the model, (2) add fake external transport mechanisms and (3) add fake intracellular transport reactions in multi-compartment models. It is an open source implementation under an open source environment.}\\
\hline
\texttt{`growMatch'} \cite{Kumar2009gf}&\texttt{COBRApy}&\texttt{Python} & {\small Developed by Kumar \textit{et al.} (2009), the \texttt{`growMatch'} algorithm identify through an \emph{optimization algorithm} the minimum number of reactions required to allow the model give flux to a selected \emph{objective function} from a universal database of stoichiometric reactions. The process is carried out one objective function per time (user defined). It is an open source implementation under an open source environment.} \\
\hline
