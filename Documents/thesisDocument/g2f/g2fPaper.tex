...
\hline
\endhead
\texttt{`SMILEY'} \cite{Reed2006}&\texttt{COBRApy}&\texttt{Python} & Developed by Reed \textit{et al.} (2006), the \texttt{`SMILEY'} algorithm identify through an \emph{optimization algorithm} the minimum number of reactions required to allow the model a selected \textit{metabolite} production from a universal database of stoichiometric reactions. The process is carried out one metabolite per time (user defined).\\
\hline
\texttt{`gap-Find/Fill'} \cite{SatishKumar2007}&\texttt{-}&\texttt{GAMS} &Developed by Kumar \textit{et al.} (2007), the \texttt{`gapFind'} and \texttt{`gapFill'} algorithms identify the metabolites (\texttt{`gapFind'}) in the metabolic network reconstruction which cannot be produced under any uptake conditions in both single and multi-compartment, and subsequently, identify the reactions (\texttt{`gapFill'}) from a customized multi-organism database that restores the connectivity of these metabolites to the original network using a \textit{optimization based} procedures. In the process, the procedures make several intra \emph{model modifications} such as: (1) modify the directionality of the reactions in the model, (2) add fake external transport mechanisms and (3) add fake intracellular transport reactions in multi-compartment models.\\
\hline
\texttt{`growMatch'} \cite{Kumar2009gf}&\texttt{COBRApy}&\texttt{Python} & Developed by Kumar \textit{et al.} (2009), the \texttt{`growMatch'} algorithm identify through an \emph{optimization algorithm} the minimum number of reactions required to allow the model give flux to a selected \emph{objective function} from a universal database of stoichiometric reactions. The process is carried out one objective function per time (user defined). \\
\hline
\texttt{`fastGapFill'} \cite{Thiele2014gf}&\texttt{openCOBRA}&\texttt{MATLAB\textcircled{c}} &\\
\texttt{`gapFill'}\cite{G2F}&\texttt{g2f}&\texttt{R} &\\
\hline
\end{longtable}
\end{center}
